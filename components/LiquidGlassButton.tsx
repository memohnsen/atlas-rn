...
const LiquidGlassButton = ({ icon, iconSize, height, width, iconColor, onPress }: LiquidGlassButtonProps) => {
  if (Platform.OS === 'ios') {
    return (
      <GlassView style={{height: height, width: width, borderRadius: 40, justifyContent: 'center', alignItems: 'center', display: 'flex' }}>
          <TouchableOpacity className='justify-center items-center h-8 w-8 rounded-full' onPress={onPress}>
