import { ProgramBuilderDay, ProgramBuilderExercise, ProgramLibraryRecord } from '@/types/workout'
...
export const buildBuilderStateFromLibraryWorkouts = (workouts: ProgramLibraryRecord[]) => {
  ...
  const grouped = new Map<string, ProgramLibraryRecord[]>()
  ...
  groupedEntries.forEach((entry) => {
    ...
    if (Object.keys(overrides).length > 0) {
      if (!day.weekOverrides) {
        day.weekOverrides = {}
      }
      if (!day.weekOverrides[entry.weekNumber]) {
        day.weekOverrides[entry.weekNumber] = {}
      }
      day.weekOverrides[entry.weekNumber][baseExercise.id] = overrides
    }
  })
